...
bot.dialog('/chatbot', [
    function (session) {
        builder.Prompts.choice(session, 'Что тебя интересует, мой маленький гетеросексуал?', "Загадочка|Шуточка|Мудрость");
    },
    function (session, results, next) {
         ...
         else if (results.response.entity == "Мудрость") {
             session.beginDialog("/wisdom");
         }
    }
]);
...
bot.dialog('/wisdom', [
    function (session) {
        builder.Prompts.text(session, "На какую тему мне сморозить мудрость?");
    },
    function (session, results) {
        request.post({
            url: 'http://95.85.50.166:8000/messages/',
            form: {message:results.response}},
            function(error, response, body) {
                resp = JSON.parse(body);
                session.send(resp.responses[0]);
            });
